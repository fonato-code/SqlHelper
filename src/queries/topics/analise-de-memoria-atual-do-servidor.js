(function (root) {
	...
	SqlHelp._qt_analise_de_memoria_atual_do_servidor = {
	  ...
	  blocks: [
		{ type: 'md', content: `
			## Querie 1 Page Life Expectancy e Grants de Memória
		...
		{ type: 'sql', title: `Query`, sql: `
			...
			WHERE	(object_name LIKE @ServiceName + 'Buffer Node%'
			...
			FROM	@Perf PC
					LEFT JOIN (
						SELECT	'Free' AS OldName, 'Free Memory' AS NewName
						UNION ALL
						SELECT 'Database', 'Database Cache Memory'
						UNION ALL
						SELECT 'Stolen', 'Stolen Server Memory'
						UNION ALL
						SELECT 'Reserved', 'Reserved Server Memory'
						UNION ALL 
			...
			SELECT	Id = 1222,
					...
					formatted_value = (
						...
						FROM	@MemTree SSM
						WHERE	Id = 1220) - SUM(formatted_value),
			...
			SELECT	[Counter Name] = P.counter_name + ISNULL(' (Node: ' + NULLIF(P.instance_name, '') + ')', ''),
					cntr_value AS Value,
					RecommendedMinimum =
						CASE
							WHEN P.counter_name = 'Page life expectancy' AND R.Value <= 300 -- no less than 300 THEN 300
							WHEN P.counter_name = 'Page life expectancy' AND R.Value > 300 THEN R.Value
							ELSE NULL
						END
			FROM	@Perf P
